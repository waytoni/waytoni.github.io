...
def generate_card_html(sermon, card_number):
    ...
    youtube_link = sermon.get('youtube_link', '')
    youtube_id = None
    message_text = None
    ...
    if youtube_link:
        ...
        elif youtube_link.startswith('https://'):
            # Extract message text from invalid YouTube link
            # Remove "https://" and replace underscores with spaces
            message_text = youtube_link[8:].replace('_', ' ')
    
    # Generate video title
    video_title = f"{title} - {date_str}" if date_str else title
    ...
    # Add video or message if available
    if youtube_id:
        card_html += f'''
    ...
    if notes_data:
        card_html += '''
                        ...
                        <div class="resources">
                            <h3>දේසනාව සඳහා පත්‍රිකා / සටහන්:</h3>'''
        ...
        for i, note in enumerate(notes_data):
            ...
            if loc and loc.lower() not in ['none', '']:
                ...
                link_text = desc if desc else "දේශනා සඳහා පත්‍රිකා / සටහන්"
                card_html += f'''
                            <a href="{loc}" 
                               target="_blank" 
                               class="resource-link">
                ...
                card_html += f'''
                            <div style="padding: 0.8rem; margin: 0.3rem 0; background: #f0f0f0; border-radius: 5px; border-left: 3px solid #ccc; color: black;">
                                {html.escape(desc)}
                            </div>'''
...
def main():
    ...
    output_path = os.path.join(vishesha_dir, 'vi_com.html')
